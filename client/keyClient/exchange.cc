...
void* KeyEx::threadHandlerMin(void* param)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (temp.end == 1) {
                input.type = SHARE_END;
                cout << "Key Generate time = " << insertTime << " s" << endl;
            }
...
void KeyEx::elimination(unsigned char* buff, int size, int index)
{
    ...
    bne = BN_new();
    bnd = BN_new();
    RSA_set0_key(rsa_, bnn, bne, bnd);

    // 	convert buffer data into big number
    BN_bin2bn(buff, size, h_);
    // 	compute inverse
    BN_mod_inverse(inv_, record_[index], bnn, ctx_);
    // 	compute (h^(-1))d mod n
    BN_mod_mul(mid_, h_, inv_, bnn, ctx_);
...
int KeyEx::verify(unsigned char* original, unsigned char* buff, int size)
{
    ...
    BN_bin2bn(buff, size, h_);
    //	compute h^e mod n */
    BN_mod_exp(mid_, h_, bne, bnn, ctx_);
...
void KeyEx::decoration(unsigned char* hash_buf, int size, unsigned char* ret_buf, int index)
{
    ...
    //	compute r^e mod n
    BN_mod_exp(mid_, record_[index], bne, bnn, ctx_);
    //	compute h*r^e mod n
    BN_mod_mul(mid_, mid_, h_, bnn, ctx_);
...
void KeyEx::newFile(int user, char* filePath, int pathSize, char* policy)
{
    ...
    bnd = BN_new();
    RSA_set0_key(rsa_, bnn, bne, bnd);
    BN_bn2bin(bne, (unsigned char*)meta + sizeof(int) * 3 + (COMPUTE_SIZE - BN_num_bytes(bne)));
    BN_bn2bin(bnn, (unsigned char*)meta + sizeof(int) * 3 + COMPUTE_SIZE + (COMPUTE_SIZE - BN_num_bytes(bnn)));
    // send meta
    BN_free(bnn);
    BN_free(bne);
    BN_free(bnd);
