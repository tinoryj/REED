...
Aont::Aont(CryptoPrimitive *cryptoObj, int type) {
	...
	for (int i = 0; i < alignedBufferSize_; i++) {
...
int Aont::encode(unsigned char* buf, int size, unsigned char* package, int* retSize, unsigned char* key, unsigned char* stub) {
	...
	if (type_ == SIM) 
		simple_encode(buf,size,package,retSize,key,stub);
	else 
		adv_encode(buf,size,package,retSize,key,stub);
	return 0;
}
...
int Aont::decode(unsigned char* package, int size, unsigned char* buf, int* retSize) {
	...
	if (type_ == SIM) 
		simple_decode(package,size, buf, retSize);
	else 
		adv_decode(package,size, buf, retSize);
	return 0;
}
